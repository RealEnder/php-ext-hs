dnl config.m4 for extension hs
...
PHP_ARG_ENABLE(hs, whether to enable heatshrink support,
[  --enable-hs           Enable heatshrink support])

if test "$PHP_HS" != "no"; then

  CFLAGS="-std=c99 -Wall"
  PHP_NEW_EXTENSION(hs, hs.c heatshrink/heatshrink_encoder.c heatshrink/heatshrink_decoder.c, $ext_shared)
